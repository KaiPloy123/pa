										...
										<?php
											...
											else
											{
												...
												if($query_rcon_server->num_rows > 0)
												{
													...
													$rcon = new Rcon($rcon_server['ip_server'], $rcon_server['port'], $rcon_server['password'], '3');
													...
													if($rcon->connect())
													{
														...
														if($query_rem_points)
														{
															$activities_action = "Buy Item";
															$time_date = date("Y-m-d H:i");
															$sql_insert_log = 'INSERT INTO activities (uid,username,action,topup_amount,date,transaction) VALUES ("'.$_SESSION['uid'].'","'.$_SESSION['username'].'","'.$activities_action.'","'.$buy['price'].'","'.$time_date.'","'.$buy['name'].'")';
